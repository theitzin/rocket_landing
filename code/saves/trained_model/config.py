...
def network_init(input_data, n_actions):
	...
	hidden = tf.layers.dense(input_data, 256, 
...
def adjust_difficulty(program_state, summary):
	...
	if np.mean(summary['landing_success_rate']) > 0.9:
		program_state['difficulty'] = min(2, program_state['difficulty'] + 0.005)
		program_state['last_difficulty_change'] = program_state['episode']

	if np.mean(summary['landing_success_rate']) < 0.7:
		program_state['difficulty'] = max(0.1, program_state['difficulty'] - 0.01)
		program_state['last_difficulty_change'] = program_state['episode']
